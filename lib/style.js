...
var layouts = [], processor, log, ratio, margin;
...
Handlebars.registerHelper('baseDim', function (size) {
  return Math.round(size * ratio) - Math.round((margin * ratio) * 2);
});

Handlebars.registerHelper('basePos', function (pos) {
  return (Math.round(pos * ratio) + Math.round(margin * ratio)) * -1;
});
